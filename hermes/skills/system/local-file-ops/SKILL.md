...
metadata:
  hermes:
    tags: [files, filesystem, cleanup, organization, duplicates, workstation, hygiene]
    category: system
    related_skills: [linux-system-health, career-artifact-builder, obsidian-memory-curator]
---
...
## Required Inputs
- Target path(s): one or more directories to inspect (e.g. `~/Downloads`, `~/Documents`, `~/hermes-mission-control`).
...
1. **Scope confirmation (always first)**
   ...
   - If target path contains `~/.hermes`, `~/.ssh`, `~/.gnupg`, `~/.config`, or `/etc`: require explicit confirmation before proceeding.
...
7. **Await approval before any mutation**
   ...
   - Log every completed action to `~/.hermes/logs/file-ops-YYYY-MM-DD.log`.
...
- Do **not** traverse or report on `~/.ssh`, `~/.gnupg`, `~/.hermes/.env`, or any path containing `secret`, `token`, `key`, `credential` in the name — skip and note.
- Do **not** delete any file that was modified in the last 7 days without a second explicit confirmation.
- Do **not** run content-hash deduplication on trees >1GB without approval (I/O cost).
- Do **not** empty a directory named after a project without confirming the project is archived or committed.
...
|---|--------|------|-------------|--------|------|
| 1 | DELETE | /full/path/file.tmp | — | Temp file, 0 bytes | Safe |
| 2 | ARCHIVE | /full/path/old-proj/ | ~/Archives/ | Inactive >180 days | Review |
...
- For project archiving, pair with `github-repo-review` to confirm the repo is committed and pushed before removing local files.
- Log files in `~/.hermes/logs/file-ops-*.log` provide an audit trail if a deletion needs to be investigated later.
